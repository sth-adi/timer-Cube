...
import { planFromFacelets, type AlgOverrides, type NavStep } from "./planner";

export type SatNavRequest = { id: number; facelets: string; overrides?: AlgOverrides };
...
ctx.onmessage = (e: MessageEvent<SatNavRequest>) => {
  const { id, facelets, overrides } = e.data;
  try {
    ctx.postMessage({ id, ok: true, step: planFromFacelets(facelets, overrides) } satisfies SatNavResponse);
  } catch (err) {
